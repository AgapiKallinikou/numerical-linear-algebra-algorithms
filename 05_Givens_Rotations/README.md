...
A naive implementation of the multiplication $A \cdot J$ would require explicitly constructing the $n \times n$ matrix $J$ and performing standard matrix multiplication, resulting in computationally heavy overhead ($\mathcal{O}(mn^2)$ flops).
...
Expanding and using asymptotic approximations for large matrices ($\sum j \approx n^2/2$ and $\sum j^2 \approx n^3/3$):
...
The standard Householder QR factorization requires $n^2(m - n/3)$ operations. 
...
The total number of applied Givens rotations is approximately $mn - n^2/2$.
